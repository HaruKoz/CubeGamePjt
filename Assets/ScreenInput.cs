...
public class ScreenInput : MonoBehaviour
{
    ...
    // テスト用
    [SerializeField]
    private Text _testText_dir; //オブジェクトの向き

    [SerializeField]
    private Text _testText;

    [SerializeField]
    private Text _testText_ptn; //キューブの出ている柄を表示するテキスト
    ...
    private void GetInputVector()
    {
        ...
        if (Application.isEditor)
        {
            ...
            {
                hit = new RaycastHit();
                if (RC.GetStartPosition(Input.mousePosition, out hit, out pos))
                {
                    HitFlg = true;
                    InputSTART = pos;
            ...
            {
                if (HitFlg && RC.GetEndPosition(Input.mousePosition, HitPoint, HitNormal, out pos))
                {
                    InputEND = pos;
                    FlickCLC();
                    HitFlg = false;
    ...
    //
    private bool ApproximatelyVec(Vector3 v1, Vector3 v2)
    {
